...
class _SignUpScreenState extends State<SignUpScreen> {
  // تعريف الـ Controllers لسحب النصوص من الحقول
  ...
  // دالة التعامل مع عملية التسجيل والربط مع الباك إيند
  Future<void> _handleSignUp() async {
    final email = _emailController.text.trim();
    final password = _passwordController.text.trim();
    final confirmPassword = _confirmPasswordController.text.trim();

    // 1. التحقق من صحة المدخلات محلياً
    ...
    try {
      // 2. إرسال الطلب للباك إيند لإنشاء الحساب
      // استبدلي IP السيرفر الخاص بك هنا
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // 3. الانتقال لشاشة التحقق وتمرير الإيميل (لحل الإيرور الذي ظهر عندك)
        if (mounted) {
          Navigator.push(
            context,
            MaterialPageRoute(
